...
| D19 | 2026-04-26 | Dashboard exposes a single Next.js route handler `POST /api/reanalyze` that spawns `python -m pipeline.run_all` as a detached subprocess. Pipeline writes progress to `app/public/data/run_status.json`; the dashboard polls that file (no API for data, only for control). Front door becomes `/library` (intake screen) → `/dashboard` (existing). Pre-stage use only; anti-goal §4.2 still bars live AWS during the live stage demo. | Demo narrative wants an "upload videos" front door + a "re-analyze" path that actually re-runs the pipeline on the canonical inputs. Doing this without a thin local-only control endpoint would force terminal commands during prep, which is operationally fragile in a 24h timeline. Strict reading of D11 (no API for data flow) is preserved — the API only triggers; dashboard still reads static files. |
